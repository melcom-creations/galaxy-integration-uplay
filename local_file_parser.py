...
class LocalParser(object):
    ...
    def _convert_data(self, data):
        # Calculate object size from the header bytes.
        if data > 256 * 256:
    ...
    def _parse_configuration_header(self, header, second_eight=False):
        try:
            ...
            # If the object size is smaller than 128 bytes, the secondary size may not use two bytes.
            if record_size - offset < 128 <= record_size:
                tmp_size -= 1
                record_size += 1

            # If parsing lands in the middle of the header, return normalized values.
            # Return the YAML size and launch_id at the end of the record as well.
            return record_size - offset, launch_id, launch_id_2, offset + tmp_size + 1
        except:
            # Something went wrong; do not crash.
            # Return zeroes so the caller can handle it later.
            # Advance 10 bytes in the configuration file to find the next game.
            return 0, 0, 0, 10
    ...
    def _get_game_name_from_yaml(self, game_yaml):
        ...
        # Try the secondary fallback path.
        if game_name.lower() in UBISOFT_CONFIGURATIONS_BLACKLISTED_NAMES:
