...
def _get_playtime_heuristics(time_stats):
    ...
    if len(time_stats) == 0:
        ...
        for st in time_stats:
            ...
            if len(time_stats) == 2:
                # Handle games that track multiple playtime modes separately.
                ...
                if (('pvp' in n1 and 'pve' in n2) or ('pve' in n1 and 'pvp' in n2)) or \
                    ...
                    cards = time_stats
        if len(cards) == 0:
            # Rank the remaining candidates using keyword matches.
            for st in time_stats:
                st['_weight'] = 0
...
def find_times(statscards: dict, game_id: Optional[str] = None) -> Tuple[Optional[int], Optional[int]]:
    ...
    # Apply the Champions of Anteria playtime override.
